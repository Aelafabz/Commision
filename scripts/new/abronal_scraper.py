...
abronal_scraper.py
──────────────────────────────────────────────────────────────────
Neo-script #0 — obtains the Abronal data.

This is the web-app-native replacement for the browser-automation
half of the old standalone `export_physician_performance.py`. It
keeps the same login -> select role -> open report -> per-physician
export flow (Playwright, headless by default), but:

  * drops files straight into data/uploads/abronal/ using the same
    "<date label> <Dr. Name>.xlsx" naming primary_reconciliation.py
    already expects (physician name is read back out of the filename
    at reconciliation time — nothing changed there),
  * takes its date range / physician filter from the web app instead
    of a Tk date picker,
  * reports progress through a `log(str)` callback so the FastAPI
    router can stream it to the browser over a websocket, exactly
    like the reconciliation pipeline does.

Credentials (BASE_URL, USERNAME, PASSWORD, ROLE) are read from a
`.env` file at the project root — see .env.example. They are never
sent to the browser or written to any exported file.
"""
...
class ScraperError(Exception):
    """Raised for any expected failure (bad config, bad date range,
    physician not found, etc.) so the router can surface a clean
...
def _resolve_targets(options: list[Physician], skip_names: list[str],
    ...
    if physicians:
        needles = [n.lower().strip() for n in physicians]
        selected = [o for o in options if o.label.lower().strip() in needles
                    or any(n in o.label.lower() for n in needles)]
        missing = [n for n in needles if not any(
            n == o.label.lower().strip() or n in o.label.lower() for o in options
        )]
        if missing:
            raise ScraperError(f"Physician(s) not found in dropdown: {', '.join(missing)}")
        return selected
